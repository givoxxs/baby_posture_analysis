...
class BabyPostureAnalysisPipeline:
    ...
    def __init__(self, model_path, scaler_path):
        ...
        # Map số sang tên lớp
        self.CLASS_LABELS = {0: "Nằm ngửa", 1: "Nằm nghiêng", 2: "Nằm sấp"}
        ...
        self.keypoints_helper = KeypointsExtractorHelper(
            self.IMPORTANT_KEYPOINTS, self.mp_pose
        )
        self.pose_scaler_helper = PoseScalerHelper(
            self.IMPORTANT_KEYPOINTS, self.mp_pose
        )
    ...
    def extract_keypoints(self, image):
        ...
        results.pose_landmarks = self.image_rotation_helper.rotate_image_baby(
            results.pose_landmarks
        )

        # Trích xuất và chuẩn hóa các keypoints sử dụng helper
        df_keypoints = self.keypoints_helper.process_keypoints(
    ...
    def detect_blanket_covering(self, results, image):
        ...
        for idx in body_indices:
            if (
                idx < len(results.pose_landmarks.landmark)
                and results.pose_landmarks.landmark[idx].visibility
    ...
    def visualize_result(
        self, image, results, predicted_class, probabilities, face_covered, is_covered
    ):
        """
        ...
        for dx, dy in [
            ...
            draw.text(
                (10 + dx, y_offset + dy), text, font=font_large, fill=shadow_color
            )
        # Vẽ text chính
        ...
        for dx, dy in [
            ...
            draw.text(
                (10 + dx, y_pos + dy), blanket_text, font=font_small, fill=shadow_color
            )
        ...
        for i, (label, prob) in enumerate(
            zip(self.CLASS_LABELS.values(), probabilities)
        ):
    ...
    def analyze_image(self, image, timestamp=None):
        ...
        try:
            ...
            face_confidence = (
                0.0  # Default value since detect_face_covering was removed
            )
            is_covered, coverage_ratio = self.detect_blanket_covering(
                results, image_rgb
            )
