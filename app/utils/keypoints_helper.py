...
class KeypointsExtractorHelper:
    ...
    def extract_and_format_keypoints(self, keypoints):
        ...
        columns_name = [] + [
            f"{landmark}_{axis}"
            for landmark in self.important_landmarks
            for axis in ["x", "y", "z"]
        ]
    ...
    def find_center_of_body(self, df_keypoints):
        ...
        """
        left_hip = (
            df_keypoints["left_hip_x"].values[0],
            df_keypoints["left_hip_y"].values[0],
        )
        right_hip = (
            df_keypoints["right_hip_x"].values[0],
            df_keypoints["right_hip_y"].values[0],
        )

        center_hip = (left_hip[0] + right_hip[0]) / 2, (left_hip[1] + right_hip[1]) / 2

        left_shoulder = (
            df_keypoints["left_shoulder_x"].values[0],
            df_keypoints["left_shoulder_y"].values[0],
        )
        right_shoulder = (
            df_keypoints["right_shoulder_x"].values[0],
            df_keypoints["right_shoulder_y"].values[0],
        )
        ...
        center = (center_hip[0] + center_shoulder[0]) / 2, (
            center_hip[1] + center_shoulder[1]
        ) / 2
        return center
    ...
    def calc_scale_value(self, df_keypoints):
        ...
        distance_between_hips = np.sqrt(
            (center_hip[0] - center_shoulder[0]) ** 2
