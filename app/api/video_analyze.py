...
class SafetyMonitor:
    ...
    def update(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.frame_history.append({
            ...
            "is_covered": analysis_result.get("analysis", {}).get("is_covered", True)  # Default to covered
        })
        ...
        prone_position_streak = self._calculate_consecutive_condition(lambda frame: frame["position"] == 2)  # 2 = Nằm sấp
        side_position_streak = self._calculate_consecutive_condition(lambda frame: frame["position"] == 1)  # 1 = Nằm nghiêng
        no_blanket_streak = self._calculate_consecutive_condition(lambda frame: not frame["is_covered"])
