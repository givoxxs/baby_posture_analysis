...
async def analyze_baby_posture(
    ...
    timestamp: Optional[str] = Query(
        None,
        description="Timestamp of the image in ISO format (YYYY-MM-DDTHH:MM:SS.sssZ). If not provided, current time will be used.",
    ),
    ...
    try:
        ...
        if not result.get("success"):
            logger.warning(
                f"Analysis failed for {file.filename}: {result.get('message')}"
            )
