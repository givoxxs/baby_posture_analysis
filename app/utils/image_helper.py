...
class Image_Rotation_Helper:
    ...
    def rotate_image_baby(self, keypoints, origin_size=(612, 408)) -> np.ndarray:
        ...
        c_shoulder = (left_shoulder.x + right_shoulder.x) / 2, (
        ...
        vector_center_to_c_shoulder = (
            c_shoulder[0] - center[0],
            c_shoulder[1] - center[1],
        )
    ...
    def calc_angle_rotate(self, OA: Tuple, OB: Tuple):
        ...
        cos_angle = dot_product / (
            normOA * normOB
        )  # cosin của góc giữa 2 vector OA và OB

        cos_angle = np.clip(
            cos_angle, -1.0, 1.0
        )  # hàm clip để giới hạn giá trị trong khoảng [-1, 1]
        angle = np.arccos(cos_angle)

        cross_product = (
            OA[0] * OB[1] - OA[1] * OB[0]
        )  # tích có hướng của 2 vector OA và OB -> để xác định chiều quay
        if cross_product < 0:
            angle = 2 * np.pi - angle
    ...
    def rotate_point(
        self, point: Tuple, center: Tuple, angle: float, origin_size=(612, 408)
    ) -> tuple:
        x, y = point.x, point.y
        center_x, center_y = center

        x_new = (
            (x - center_x) * np.cos(np.radians(angle)) * origin_size[0]
            - (y - center_y) * np.sin(np.radians(angle)) * origin_size[1]
            + center_x * origin_size[0]
        )
        y_new = (
            (x - center_x) * np.sin(np.radians(angle)) * origin_size[0]
            + (y - center_y) * np.cos(np.radians(angle)) * origin_size[1]
            + center_y * origin_size[1]
        )
